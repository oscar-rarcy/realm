# Castle Wall Feature Image Generation Prompt

Generate Realm image sheets for **castle wall feature**.

## Art Brief

- Asset group: features
- Asset id: castle_wall
- Visual design: transparent blocking castle wall feature anchored to tile centre
...
Generate **one sprite for each listed state or variant**. There are 7 item(s). Each image may contain at most **16 items** in a **4 by 4** grid.
...
- row 1, column 1: `base_clear` - default castle wall segment
- row 1, column 2: `rain_wet_dark` - castle wall darkened by rain
- row 1, column 3: `frost` - castle wall with frost on stone edges
- row 2, column 1: `snow_light` - castle wall with light snow on upper surfaces
- row 2, column 2: `snow_heavy` - castle wall with heavier settled snow while joins stay readable
- row 2, column 3: `damaged` - damaged cracked castle wall segment
- row 3, column 1: `broken` - broken castle wall segment with rubble
...
Generate Realm castle wall feature image sheets. Use transparent-background anchored sprites with no full ground tile, consistent anchor position, and readable silhouette. Create one sprite for each of the 7 listed states or variants. The default state is base_clear.  Order items left to right and top to bottom within each sheet. Use clean readable small-RTS art, stable scale, clear gutters, no text labels, no numbers, no watermark, and no cropped artwork. Use transparent background, or a single flat #ff00ff magenta background if transparency is not available.

Slot order:
- Grid: 3 by 3
  - row 1, column 1: default castle wall segment
  - row 1, column 2: castle wall darkened by rain
  - row 1, column 3: castle wall with frost on stone edges
  - row 2, column 1: castle wall with light snow on upper surfaces
  - row 2, column 2: castle wall with heavier settled snow while joins stay readable
  - row 2, column 3: damaged cracked castle wall segment
  - row 3, column 1: broken castle wall segment with rubble
