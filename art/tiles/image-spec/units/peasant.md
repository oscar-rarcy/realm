...
Generate **one frame for each state**. There are 2 state(s). Each image may contain at most **16 states** in a **4 by 4** grid.

### Sheet

Use a **2 by 1** grid for this sheet.

- row 1, column 1: `dead` - dead villager body lying on the ground with clothing and simple tools still intact
- row 1, column 2: `decayed` - human skeleton remains, with small clothing and tool scraps still readable
