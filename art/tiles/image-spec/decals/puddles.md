# Puddles Decal Image Generation Prompt

Generate Realm image sheets for **puddles decal**.

## Art Brief

- Asset group: decals
- Asset id: puddles
- Visual design: transparent flat puddle overlay for rain, marsh, mud, and road edges
- Projection: transparent low or flat overlay that sits on top of ground
- Footprint: 1 by 1 tile
- Directions: tile
- Default state: `rain_puddle_small`
...
- row 1, column 1: `rain_puddle_small` - small rain puddle
- row 1, column 2: `rain_puddle_large` - larger rain puddle
- row 2, column 1: `storm_puddle_choppy` - storm puddle with choppy surface
- row 2, column 2: `frozen_puddle` - frozen puddle with thin ice
...
Generate Realm puddles decal image sheets. Use transparent low/flat overlay decals that sit on the ground and do not imply an independent blocking object. Create one decal for each of the 4 listed states or variants. The default state is rain_puddle_small.  Order items left to right and top to bottom within each sheet. Use clean readable small-RTS art, stable scale, clear gutters, no text labels, no numbers, no watermark, and no cropped artwork. Use transparent background, or a single flat #ff00ff magenta background if transparency is not available.

Slot order:
- Grid: 2 by 2
  - row 1, column 1: small rain puddle
  - row 1, column 2: larger rain puddle
  - row 2, column 1: storm puddle with choppy surface
  - row 2, column 2: frozen puddle with thin ice
