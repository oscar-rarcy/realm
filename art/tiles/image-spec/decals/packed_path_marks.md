# Packed Path Marks Decal Image Generation Prompt

Generate Realm image sheets for **packed path marks decal**.

## Art Brief

- Asset group: decals
- Asset id: packed_path_marks
- Visual design: transparent packed dirt path fragments for medium wear
- Projection: transparent low or flat overlay that sits on top of ground
- Footprint: 1 by 1 tile
- Directions: tile
- Default state: `short_fragment`
...
- row 1, column 1: `short_fragment` - short packed path fragment
- row 1, column 2: `straight_fragment` - straight packed path fragment
- row 2, column 1: `corner_fragment` - corner or bend packed path fragment
- row 2, column 2: `intersection_fragment` - small packed path intersection fragment
...
Generate Realm packed path marks decal image sheets. Use transparent low/flat overlay decals that sit on the ground and do not imply an independent blocking object. Create one decal for each of the 4 listed states or variants. The default state is short_fragment.  Order items left to right and top to bottom within each sheet. Use clean readable small-RTS art, stable scale, clear gutters, no text labels, no numbers, no watermark, and no cropped artwork. Use transparent background, or a single flat #ff00ff magenta background if transparency is not available.

Slot order:
- Grid: 2 by 2
  - row 1, column 1: short packed path fragment
  - row 1, column 2: straight packed path fragment
  - row 2, column 1: corner or bend packed path fragment
  - row 2, column 2: small packed path intersection fragment
