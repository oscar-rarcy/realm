# Wheel Ruts Decal Image Generation Prompt

Generate Realm image sheets for **wheel ruts decal**.

## Art Brief

- Asset group: decals
- Asset id: wheel_ruts
- Visual design: transparent wheel rut overlay for roads, yards, farms, and transport paths
- Projection: transparent low or flat overlay that sits on top of ground
- Footprint: 1 by 1 tile
- Directions: tile
- Default state: `light_ruts`
...
- row 1, column 1: `light_ruts` - light wheel ruts
- row 1, column 2: `deep_ruts` - deep wheel ruts
- row 2, column 1: `muddy_ruts` - muddy wheel ruts
- row 2, column 2: `snow_ruts` - wheel ruts through snow
...
Generate Realm wheel ruts decal image sheets. Use transparent low/flat overlay decals that sit on the ground and do not imply an independent blocking object. Create one decal for each of the 4 listed states or variants. The default state is light_ruts.  Order items left to right and top to bottom within each sheet. Use clean readable small-RTS art, stable scale, clear gutters, no text labels, no numbers, no watermark, and no cropped artwork. Use transparent background, or a single flat #ff00ff magenta background if transparency is not available.

Slot order:
- Grid: 2 by 2
  - row 1, column 1: light wheel ruts
  - row 1, column 2: deep wheel ruts
  - row 2, column 1: muddy wheel ruts
  - row 2, column 2: wheel ruts through snow
