# Cobble Patches Decal Image Generation Prompt

Generate Realm image sheets for **cobble patches decal**.

## Art Brief

- Asset group: decals
- Asset id: cobble_patches
- Visual design: transparent cobble or road creep patches for high wear
- Projection: transparent low or flat overlay that sits on top of ground
- Footprint: 1 by 1 tile
- Directions: tile
- Default state: `small_patch`
...
- row 1, column 1: `small_patch` - small cobble patch
- row 1, column 2: `edge_patch` - cobble edge patch
- row 2, column 1: `dense_patch` - dense cobble patch
- row 2, column 2: `broken_patch` - broken uneven cobble patch
...
Generate Realm cobble patches decal image sheets. Use transparent low/flat overlay decals that sit on the ground and do not imply an independent blocking object. Create one decal for each of the 4 listed states or variants. The default state is small_patch.  Order items left to right and top to bottom within each sheet. Use clean readable small-RTS art, stable scale, clear gutters, no text labels, no numbers, no watermark, and no cropped artwork. Use transparent background, or a single flat #ff00ff magenta background if transparency is not available.

Slot order:
- Grid: 2 by 2
  - row 1, column 1: small cobble patch
  - row 1, column 2: cobble edge patch
  - row 2, column 1: dense cobble patch
  - row 2, column 2: broken uneven cobble patch
