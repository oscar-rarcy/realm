# Rocky Ground Image Generation Prompt

Generate Realm image sheets for **rocky ground**.

## Art Brief

- Asset group: grounds
- Asset id: rocky
- Visual design: top-down square rocky floor used under mountains, boulders, and mineral deposits
...
Generate **one tile for each listed state or variant**. There are 5 item(s). Each image may contain at most **16 items** in a **4 by 4** grid.
...
Slab side material for `rocky`: cool grey-brown stone side material, with larger rock planes and cracks continuing into the bevel.
...
- row 1, column 1: `base_clear` - default rocky ground with small cracks and stone texture
- row 1, column 2: `rain_wet_dark` - rocky ground darkened by rain
- row 1, column 3: `frost` - rocky ground with frost in cracks and shaded crevices
- row 2, column 1: `snow_light` - rocky ground with light snow in crevices
- row 2, column 2: `snow_heavy` - rocky ground with heavier snow while rocks stay readable
...
Generate Realm rocky ground image sheets. Use top-down square 3D terrain slabs that fill the whole square and match the approved Realm grass reference geometry: continuous top material, subtle bevel, worn corners, and dark contact shadow; side material guidance for rocky: cool grey-brown stone side material, with larger rock planes and cracks continuing into the bevel. Create one tile for each of the 5 listed states or variants. The default state is base_clear.  Order items left to right and top to bottom within each sheet. Use clean readable watercolor map-ground styling, stable scale, clear gutters, no text labels, no numbers, no watermark, and no cropped artwork. Use opaque full-square slab art in each cell, with the same physical 3D terrain-tile sides, muted chipped worn corners, and dark contact shadow as the approved Realm grass reference; keep clear gutters between cells.

Slot order:
- Grid: 3 by 2
  - row 1, column 1: default rocky ground with small cracks and stone texture
  - row 1, column 2: rocky ground darkened by rain
  - row 1, column 3: rocky ground with frost in cracks and shaded crevices
  - row 2, column 1: rocky ground with light snow in crevices
  - row 2, column 2: rocky ground with heavier snow while rocks stay readable
