# Tundra Ground Image Generation Prompt

Generate Realm image sheets for **tundra ground**.

## Art Brief

- Asset group: grounds
- Asset id: tundra
- Visual design: top-down square snow-biome floor with seasonal thaw/refreeze identity
- Projection: top-down square source tile
- Footprint: 1 by 1 tile
- Directions: tile
- Default state: `tundra_spring_thaw`
...
Generate **one tile for each listed state or variant**. There are 6 item(s). Each image may contain at most **16 items** in a **4 by 4** grid.
...
Slab side material for `tundra`: mixed cold grey-brown earth, dull moss, and snow/frost side material; the side should show the same patchy thaw/refreeze mix as the top state.
...
- row 1, column 1: `tundra_spring_thaw` - snow-biome floor in spring thaw, wet tundra and melting snow patches
- row 1, column 2: `tundra_summer_bare_ground` - snow-biome floor in summer, bare tundra ground with hardy vegetation
- row 1, column 3: `tundra_autumn_refreeze` - snow-biome floor in autumn refreeze, frost returning to tundra
- row 2, column 1: `tundra_winter_snow` - snow-biome floor in winter, snow-covered tundra with cold blue shadows
- row 2, column 2: `snowfall_frame_1` - tundra ground, falling snow interaction frame 1 with small snow puffs or fresh specks
- row 2, column 3: `snowfall_frame_2` - tundra ground, falling snow interaction frame 2 with shifted snow puffs or fresh specks
...
Generate Realm tundra ground image sheets. Use top-down square 3D terrain slabs that fill the whole square and match the approved Realm grass reference geometry: continuous top material, subtle bevel, worn corners, and dark contact shadow; side material guidance for tundra: mixed cold grey-brown earth, dull moss, and snow/frost side material; the side should show the same patchy thaw/refreeze mix as the top state. Create one tile for each of the 6 listed states or variants. The default state is tundra_spring_thaw.  Order items left to right and top to bottom within each sheet. Use clean readable watercolor map-ground styling, stable scale, clear gutters, no text labels, no numbers, no watermark, and no cropped artwork. Use opaque full-square slab art in each cell, with the same physical 3D terrain-tile sides, muted chipped worn corners, and dark contact shadow as the approved Realm grass reference; keep clear gutters between cells.

Slot order:
- Grid: 3 by 2
  - row 1, column 1: snow-biome floor in spring thaw, wet tundra and melting snow patches
  - row 1, column 2: snow-biome floor in summer, bare tundra ground with hardy vegetation
  - row 1, column 3: snow-biome floor in autumn refreeze, frost returning to tundra
  - row 2, column 1: snow-biome floor in winter, snow-covered tundra with cold blue shadows
  - row 2, column 2: tundra ground, falling snow interaction frame 1 with small snow puffs or fresh specks
  - row 2, column 3: tundra ground, falling snow interaction frame 2 with shifted snow puffs or fresh specks
