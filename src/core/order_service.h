...
// Domain-order service for validated command execution. The service owns issuer
// validation; callers should not mutate entity order state directly.
...
ServiceResult startGarrison(Game& game, const WorldIndex& world, EventSink& events, PlayerId issuer, const Selection& selection, EntityId targetId);
