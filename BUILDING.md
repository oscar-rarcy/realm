...
```sh
brew install emscripten     # or the emsdk; needs a wasm-enabled LLVM
make web                    # -> web/index.html + .js/.wasm/.data (~4 MB)
python3 -m http.server 8080 -d web   # file:// can't fetch wasm — serve it
```

Same sources, same SDL shim — Emscripten's SDL2/SDL2_ttf ports supply the
platform, ASYNCIFY lets the game's ordinary blocking loops yield to the
browser, and saves/replays/config persist in IndexedDB (IDBFS). JetBrains
Mono (OFL) is bundled in `web/fonts/` and preloaded at `/fonts`.
No multiplayer in a tab (no raw TCP) — the splash hides the entry.

Homebrew note: if `emcc` fails its sanity check complaining about
`/usr/bin/clang`, its autogenerated config guessed the wrong LLVM — point
`LLVM_ROOT` in `$(brew --prefix emscripten)/libexec/.emscripten` at
`.../libexec/llvm/bin` (and `BINARYEN_ROOT` at `.../libexec/binaryen`).
